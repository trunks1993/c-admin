...
const Comp = (props) => {
  ...
  console.log("Comp -> path", path);
  const selectedKey = keys.find((p) => path === p || path.includes(p));
  console.log("Comp -> selectedKey", selectedKey);
    ...
    <Header>
      <div className="layout_header-main">
        ...
        >
          ...
          <Menu.Item
            onClick={(e) => window.open("http://www.yunjinshuke.com/file.html")}
          >
            文档中心
          </Menu.Item>
        </Menu>
        ...
        >
          <Menu.Item
            key="/admin/message"
            onClick={(e) => history.push("/admin/message")}
          >
            <img className="layout_header-icon" src={icon2} />
            <span>消息</span>
          </Menu.Item>
          <Menu.Item key="/admin/car" onClick={(e) => history.push("/admin/car")}>
            <Badge dot={!!carCount}>
